...
* Type System
LLVM type 系统是IR的最重要特征之一。 type 确定了，可以直接在IR上执行许多优化，而不必在转换之前在侧面进行额外的分析。强大的类型系统使读取生成的代码更容易，

** Void Type
void type 不表示任何值而且没有size。
...
#+END_SRC
** Function Type
可以将函数类型视为函数签名。 它由返回类型和参数类型列表组成。 函数类型的返回类型是void类型或first class type，label和metadata type除外。

语法：
#+BEGIN_SRC
<returntype> (<parameter list>)
#+END_SRC
<parameter list>是由逗号分隔的类型标识符。若参数list包含省略号，则表示该函数拥有可变的参数个数，可变参数函数可以通过variable argument handling intrinsic函数
访问它们的参数，<returntype>可以是除了label和metadata外的任何类型。
...
** First Class Type
这种类型的值是只能由instruction生成的值。（通常包含可以作为函数返回值的类型，可以作为参数传递给函数的类型）

*** Single Value Types
从CodeGen的角度来看，这些是在寄存器中有效的类型。
**** Integer Type
整数类型是一种非常简单的类型，它简单地为所需的整数类型指定任意位宽。 可以指定从1位到2^23-1（约800万）的任何位宽度。
...
|-----------+--------------------------------------------------------------------------|
| half      | 16位浮点值                                                               |
| bfloat    | 16位浮点值（7位有效）。提供与float相同的指数位数，因此它与动态范围匹配， |
|           | 但精度大大降低。在Intel的AVX-512 BF16扩展和Arm的ARMv8.6-A扩展中使用。    |
| float     | 32位浮点值                                                               |
| double    | 64位浮点值                                                               |
| fp128     | 128位浮点值（112位有效位）                                               |
| x86_fp80  | 80位浮点值（X87）                                                        |
| ppc_fp128 | 128位浮点值（2个64位）                                                   |
|-----------+--------------------------------------------------------------------------|
half，float，double和fp128的二进制格式分别对应于IEEE-754-2008的binary16，binary32，binary64和binary128规范。

**** X86_mmx Type
x86_mmx类型表示x86计算机上MMX寄存器中保存的值。这种类型允许的操作非常有限：参数和返回值，load和store以及bitcast。 用户指定的MMX指令表示为带有此类型的参数或
结果的intrinsic或asm调用。 没有这种类型的数组，向量或常量。
...
**** Pointer Type
指针类型用于指定memory位置。 指针通常用于引用memory中的对象。

指针类型可能具有可选的地址空间属性，该属性定义了指向对象所驻留的编号地址空间。 默认地址空间编号为零。 非零地址空间的语义是target-specific。

请注意，LLVM不允许指向void（void *）的指针，也不允许指向标签（label *）的指针。请改用i8 *。
...
**** Vector Type
向量类型是表示元素向量的简单派生类型。 使用单个指令（SIMD）并行操作多个基本数据时，将使用向量类型。 向量类型需要size（元素个数），基础原始数据类型和可扩展属性，
以表示在编译时实际硬件向量长度未知的向量。
...
元素个数是一个大于0的恒定整数值；elementtype可以是任何整数，浮点数或指针类型。不允许size为零的向量。对于可扩展向量，元素总数是指定元素数量的常数倍（称为vscale）；
 vscale是一个正整数，在编译时未知，并且和在运行时所有可扩展(scalable)向量的硬件相关常数相同。 因此，即使在运行时才能确定确切的字节size，IR中特定scalable vector类型
的大小也不变。
...
*** Token Type
当一个值与一条指令相关联时使用token type，但该值的所有使用都不得试图introspect or obscure it。因此，phi或select不适合与token type一起使用。
...
*** Metadata Type
元数据类型表示嵌入式元数据。 除函数参数外，不能从元数据创建派生类型。
...
*** Aggregate Types
Aggregate types是派生类型的子集，可以包含多个成员类型。数组和结构是aggregate types。向量不被视为aggregate types。

**** Array Type
数组类型是一种非常简单的派生类型，可以将元素顺序排列在memory中。数组类型需要size（元素个数）和基础数据类型。

语法：
#+BEGIN_SRC
[<# elements> x <elementtype>]
#+END_SRC
元素个数是一个恒定的整数值； elementtype可以是任何具有size的类型。
...
#+END_SRC
可以在长度为零的数组类型的LLVM中实现一维“可变大小数组”寻址。

**** Structure Type
结构类型用于表示内存中数据成员的集合。结构的元素可以是具有size的任何类型。

使用load和store结合getelementptr指令获取指向字段的指针来访问内存中的structure。使用extractvalue和insertvalue指令访问寄存器中的structure。

structure可以是packed结构，表示结构的alignment是一个字节，并且成员之间没有填充。在未压缩的structure中，将按module中DataLayout字符串的定义插入field types之间的填充，
这要求与底层的code generator的期望相吻合。

结构可以是literal或identified。literal结构是与其他类型内联定义的（例如{i32，i32} *），而identified structure总是在顶层使用name进行定义。literal structure的内容是唯一的，
并且由于无法更改，因此不能是递归的或不透明的（opaqued）。标识的类型可以是递归的，可以是不透明的，并且从不唯一。
...
不透明结构类型用于表示没有指定主体的命名结构类型。例如，这对应于C语言中的forward declared structure。
...
* Constants
LLVM 有几种基本的constant类型。

** Simple Constants

*** Boolean Constants
true 和 false都是i1类型的合法常数取值。

*** Integer Constants
标准整数（例如4）是integer type的常数，负数可以和integer types一起使用。

*** Float-pointing constants
浮点常数使用标准的十进制表示法（例如123.421），指数表示法（例如1.23421e+2）或更精确的十六进制表示法。 汇编器需要浮点常量的精确十进制值。
例如，汇编程序接受1.25，但拒绝1.3，因为1.3是二进制表示的重复的不精确的。 浮点常量必须具有浮点类型。

*** Null pointer constants
null标识被识别为空指针常量，并且必须是指针类型。

*** Token constants
none标识被识别为空token常量，并且必须是token类型。

常数的一种非直观表示法是浮点常数的十六进制形式。例如，格式double 0x432ff973cafa8000等同于（但比其难读）double 4.5e + 15。 唯一需要十六进制浮点常量的场合
（以及由反汇编程序生成它们的唯一场合）是在必须生成浮点常量但不能将其表示为十进制浮点数的合适数量。例如，NaN，无穷大和其他特殊值均以其IEEE十六进制格式表示，
因此汇编和反汇编不会造成常量中的任何位改变。

没有x86_mmx类型的常量。

** Complex Constants
复合常量是简单常量和较小复合常量的（潜在递归）组合。

*** Structure constants
用类似于结构类型定义的符号表示结构常数（用逗号分隔的成员列表，用大括号（{}）包围）。 例如：“ {i32 4，float 17.0，i32 * @G}”，其中“ @G”被声明为“ @G = external global i32”。
结构常数必须具有结构类型，并且成员的数量和类型必须与该类型对应的成员匹配。

*** Array constants
数组常量用类似于数组类型定义的符号表示（用逗号分隔的成员列表，用方括号（[]）包围）。 例如：“ [i32 42，i32 11，i32 74]”。 数组常量必须具有数组类型，并且元素的数量和类型必
须与该类型指定的匹配。 作为一种特殊情况，字符数组常量也可以使用c前缀表示为双引号字符串。 例如：c“Hello World \ 0A \ 00”。

*** Vector constants
向量常量用类似于向量类型定义的符号表示（用逗号分隔的元素列表，用小于/大于（<>）包围）。 例如：“ <i32 42，i32 11，i32 74，i32 100>”。 向量常量必须具有向量类型，并且元素的
数量和类型必须与该类型指定的元素匹配。

*** Zero initialization
字符串“zeroinitializer”可用于零初始化任何类型，包括标量和aggregate类型。 这通常用于避免打印大的零初始值（例如对于大型数组），并且始终完全等同于使用显式的零初始值。

*** Metadata node
metadata node是没有类型的常量元组。例如：“！{！0，！{！2，！0}，！“ test”}“。 元数据可以引用常量值，例如：“！{！0，i32 0，i8 * @global，i64（i64）* @function，！“ str”}”。
与被当作指令流一部分的其他类型常量不同，元数据是提供附加信息（例如调试信息）的地方。

** Global Variable and Function Addresses
全局变量和函数的地址始终是隐式有效（链接时）常量。当使用全局标识符并且始终具有指针类型时，将明确引用这些常量。例如，以下是合法的LLVM文件：
#+BEGIN_SRC
@X = global i32 17
@Y = global i32 42
...
** Undefined Values
字符串“undef”可在任何需要常量的地方使用，表示该值的用户可能会收到未指定的位模式。 未定义的值可以是任何类型（“ label”或“ void”除外），并且可以在允许使用常量的任何地方使用。

因为undefined values可以取任何值，编译器会基于其中某一种可能的取值来做优化和转换，这种优化和转换在LLVM中是合法的。

** Poison Values
poison value是错误操作的结果。为了促进推测执行，当提供了非法操作数时，许多指令不会立即调用undefined behavior，而是返回poison value。 字符串“poison”可以在任何需要常量的地方
使用。

** Well-Defined Values
给定程序执行的值，如果该值没有undef bit并且在执行中不是poison，该值就是well defined value。 如果aggregate value或vector的成员是well defined，则它自身就是well defined。

一个single value的常数，non-vector 类型是well defined 如果它既不是undef constant也不是poison constant。freeze instruction的结果是well defined无论它的operand是什么。

** Addresses of Basic Blocks
#+BEGIN_SRC
blockaddress(@function, %block)
#+END_SRC
“blockaddress”常量计算指定函数中指定basic block的地址，并且始终为i8 *类型。取entry block的地址是非法的。

这个值只有在作为indirectbr或者callbr指令的operand，或者和null比较时才有defined behavior。

** DSO Local Equivalent
#+BEGIN_SRC
dso_local_equivalent @func
#+END_SRC
“dso_local_equivalent”常量表示函数与给定函数在功能上等价，但始终定义在当前链接单元。其结果指针与underlying function具有相同的类型。允许但不要求结果指针与指向函数的指针不同，
并且结果指针在不同的转换单元中可能具有不同的值。

** Constant Expressions
常量表达式用于允许将包含其他常量的表达式用作常量。常量表达式可以是任何first class type，并且可以涉及任何没有side effects的LLVM操作（例如不支持load和call）。
...
4、fptrunc (CST to TYPE)
将浮点常量截断为另一种浮点类型。 CST的size必须大于TYPE的size。 两种类型都必须是浮点数。

5、fpext (CST to TYPE)
将浮点数常量扩展为另一种浮点类型。 CST的size必须小于或等于TYPE的size。 两种类型都必须是浮点数。

6、fptoui (CST to TYPE)
将浮点常量转换为相应的无符号整型常量。TYPE必须是标量或向量整数类型。 CST必须为标量或向量浮点类型。 CST和TYPE都必须是标量，或者成员数量相同的向量。如果该值不适合整数类型，
则结果为poison value。

7、fptosi (CST to TYPE)
同6，只是转换为相应的有符号整型常量。

8、uitofp (CST to TYPE)
将无符号整数常量转换为相应的浮点常量。 TYPE必须是标量或向量浮点类型。CST必须为标量或向量整数类型。 CST和TYPE都必须是标量，或者成员数量相同的向量。

9、sitofp (CST to TYPE)
同8，但是将有符号整数常量转换成相应的浮点常量。

10、ptrtoint (CST to TYPE)
在常数上执行ptrtoint操作。

11、inttoptr (CST to TYPE)
在常数上执行inttoptr操作。

12、bitcast (CST to TYPE)
将常量CST转换为另一个TYPE。 操作数的约束与bitcast instruction的约束相同。

13、addrspacecast (CST to TYPE)
将常量指针或指针CST的常量向量转换为其他地址空间中的另一个TYPE。 操作数的约束与addrspacecast指令的约束相同。

14、getelementptr (TY, CSTPTR, IDX0, IDX1, ...), getelementptr inbounds (TY, CSTPTR, IDX0, IDX1, ...)

对常量执行getelementptr操作。 与getelementptr指令一样，索引列表可能包含一个或多个索引。

15、select (COND, VAL1, VAL2)
对常量执行select操作。

16、icmp COND (VAL1, VAL2)
对常量执行icmp操作。

17、fcmp COND (VAL1, VAL2)
对常量执行fcmp操作。

18、extractelement (VAL, IDX)
对常量执行extractelement操作。

19、insertelement (VAL, ELT, IDX)
对常量执行insertelement操作。

20、shufflevector (VEC1, VEC2, IDXMASK)
对常量执行shufflevector操作。

21、extractvalue (VAL, IDX0, IDX1, ...)
对常量执行extractvalue操作。 索引列表的理解方式与“getelementptr”操作中的索引类似。 必须至少指定一个索引值。

22、insertvalue (VAL, ELT, IDX0, IDX1, ...)
同21，但是对常量执行insertvalue操作。

23、OPCODE (LHS, RHS)
执行针对LHS和RHS常数的指定操作。OPCODE可以是任何二进制或按位二进制操作。对操作数的约束与相应指令的约束相同（例如，不允许对浮点值进行按位运算）。

* Other Values
** Inline Assembler Expressions
LLVM通过使用特殊值来支持内联汇编程序表达式（与module-level内联汇编相反）。此值将内联汇编程序表示为模板字符串（包含要发出的指令），操作数约束列表（存储为字符串），
指示内联asm表达式是否有副作用的flag以及指示是否有副作用的flag。包含asm的函数需要保守地对齐其堆栈。
* Metadata
LLVM IR允许在程序中将metadata attached to instruction，这样能传递额外的代码信息给optimizer和code generator。metadata的一个典型应用是源码级的debug信息。
有两种metadata primitives: strings和nodes。

metadata没有类型，也不是一个值，如果被call instruction引用，它将使用metadata type。

所有的metadata以感叹号!开头。

** Metadata Nodes and Metadata Strings
metadata string是用双引号引起来的字符串。通过用“\xx”转义不可打印的字符，它可以包含任何字符，其中“xx”是两位十六进制代码。例如：“!"test \ 00"”。

metadata nodes用类似于结构常数的符号表示（用逗号分隔的成员列表，用大括号括起来，并带有感叹号）。metadata node的操作数可以是任何值。例如：
#+BEGIN_SRC
!{ !"test\00", i32 10}
#+END_SRC
不是唯一的metadata nodes可以使用distinct关键字标识，例如：
#+BEGIN_SRC
!0 = distinct !{!"test\00", i32 10}
#+END_SRC
当不会根据节点的内容合并节点时，使用distinct nodes很有用。当metadata operands更改，转换导致唯一性冲突时，也会发生这种情况。

named metadata是metadata nodes的集合，可以在module symbol talbe 中查找。 例如：
#+BEGIN_SRC
!foo = !{!4, !3}
#+END_SRC

metadata可以用作函数参数。下例，llvm.dbg.value 使用三个metadata参数：
#+BEGIN_SRC
call void @llvm.dbg.value(metadata !24, metadata !25, metadata !26)
#+END_SRC

metadata可以附加到指令。 下面，metadata!21使用!dbg标识符附加到add指令：
#+BEGIN_SRC
%indvar.next = add i64 %indvar, 1, !dbg !21
#+END_SRC

metadata也可以附加到函数或全局变量。 下面，元数据!22附加到f1和f2函数，并使用!dbg标识符附加全局变量g1和g2：
#+BEGIN_SRC
...
#+END_SRC
需要进行转换，以删除不知道或无法保留的任何metadata attachment。 当前，!type和!absolute_symbol附加到全局变量的metadata附件是一个例外，除非全局变量本身被删除，
否则不能无条件删除它们。

除debug metadata（名称为!llvm.dbg.*的metadata）外，使用named metadata附加到module的元数据不得删除。

以下是有关optimizer和code generator识别特定元数据节点的更多信息。

*** Specialized Metadata Nodes
specialized metadata nodes是metadata中的自定义数据结构（与通用元组相反）。 它们的字段已标记，可以按任何顺序指定。

这些原本不是以调试信息为中心，但是目前所有specialized metadata nodes都与调试信息相关。详细的内容可参看https://llvm.org/docs/LangRef.html#specialized-metadata-nodes

*** ‘tbaa’ Metadata
在LLVM IR中，memory没有类型，因此LLVM自己的类型系统不适合进行基于类型的别名分析（TBAA）。而是将metadata添加到IR中，以描述高级语言的类型系统。这可以用于实现C/C++严格类型
别名规则，也可以用于实现其他语言的自定义别名分析。

LLVM的TBAA系统的描述分为两个部分：1、semantics讨论高级问题;2、representation讨论各种实体的metadata编码。

始终可以将任何TBAA节点跟踪到“根” TBAA节点。具有不同根的TBAA节点具有未知的别名关系，LLVM保守地推断它们之间的MayAlias。 本节中提到的规则仅适用于位于在同一根目录下的TBAA节点。
...
llvm.memcpy通常用于实现类似于C或类似语言中的aggregate赋值操作，但是它被定义为复制memory的连续区域，这对于由于填充而包含空隙的aggregate类型是非必要的。此外，它不包含有关
aggregate字段的任何TBAA信息。

!tbaa.struct元数据可以描述memcpy中的哪些memory子区域正在填充以及该结构的TBAA标签是什么。

!tbaa.struct metadata nodes是三个一组的操作数列表。第一个操作数以字节为单位给出字段的字节偏移量，第二个操作数以字节为单位给出其size，第三个操作数给出其tbaa标签。 例如：
#+BEGIN_SRC
!4 = !{ i64 0, i64 4, !1, i64 8, i64 4, !2 }
#+END_SRC
这描述了具有两个字段的结构。第一个偏移量为0字节，大小为4字节，tbaa标签为!1。 第二个偏移量为8个字节，大小为4个字节，tbaa标签为!2。

这些字段不必是连续的。在此示例中，两个字段之间有4字节的间隙。 该间隙表示填充，填充不携带有用的数据，不需要保留。

*** ‘noalias’ and ‘alias.scope’ Metadata

* Intrinsic Global Variables
LLVM具有许多magic全局变量，这些变量包含影响代码生成或其他IR语义的数据。此类的所有全局变量都有一部分被指定为llvm.metadata。本节以及所有以llvm开头的全局变量是LLVM保留字段。

** llvm.used
llvm.used全局变量是一个有appending linkage的数组，该数组包含指向named全局变量，函数和别名的指针列表，这些指针可以有包含由bitcast或getelementptr形成的指针。
...
** llvm.compiler.used
llvm.compiler.used与llvm.used相同，只不过它防止编译器接触该符号。此结构很少被用到，不应出现在souce中。
...
#+END_SRC
llvm.global_ctors数组包含构造函数，优先级以及关联的全局变量或函数。加载模块时，将按优先级的升序（由低到高）调用此数组引用的函数。 没有定义具有相同优先级的函数。

如果第三个字段为非null，并且指向全局变量或函数，则只有在不丢弃当前模块中的关联数据的情况下，初始化函数才会运行。
...
#+END_SRC
llvm.global_dtors数组包含析构函数，优先级以及关联的全局变量或函数。 卸载模块时，将按优先级从高到低的顺序调用此数组引用的函数。 没有定义具有相同优先级的函数的顺序。

如果第三个字段为非null，并且指向全局变量或函数，则只有在不丢弃当前模块中的关联数据的情况下，析构函数才会运行。

* Instruction Reference
LLVM 的指令包含：terminator instructions，binary instruction，bitwise binary instruction，memory instructions，other instruction。

** terminator instruction
每个basic block以terminator instruction为结尾，terminator instruction指明当前block执行完成后接下来应该执行哪个block。terminator instruction生成一个void value，它们
产生control flow，而不是valuess（invoke指令除外）。
...
#+END_SRC
ret指令用于将控制流（以及可选的值）从函数返回给调用者。

ret指令有两种形式：一种返回值，然后产生控制流，另一种仅产生控制流。

ret指令可以选择接受一个参数，即返回值。 返回值的类型必须是first class类型。

当执行ret指令时，控制流返回到调用函数的context。如果caller是call指令，则在call之后的指令处继续执行。如果caller是invoke指令，则在正常目标块的开头继续执行。如果指令返回一个值，
则该值将设置call或invoke指令的返回值。
...
#+END_SRC
br指令用于使控制流转移到当前function中的另一个basic block。 该指令有两种形式，分别对应于条件分支和无条件分支。

br指令的条件分支形式采用一个i1值和两个label值。br指令的无条件形式将单个label值作为目标。

执行有条件的br指令时，将评估i1参数。如果该值为true，则控制转到iftrue标签。 如果cond为false，则控制流向iffalse标签。如果cond是poison或undef，则该指令导致undefined behavior。
...
#+END_SRC
switch指令用于将控制流转移到几个不同的位置之一。它是对br指令的推广，它允许控制流到达多个分支。

switch指令使用三个参数：一个整型比较值value，默认的label取值，以及一组比较值常量和label对。该表不允许包含重复的常量条目。

switch指令指定value和destination的表。当执行switch指令时，将在该表中搜索给定值。 如果找到该值，则控制流转移到相应的目的地；否则，控制流将转移到默认目标。
如果value是poison或undef，则此指令具有未定义的行为。
...
#+END_SRC
indirectbr指令在当前函数中实现了label的间接分支，该label的地址由address指定。地址必须从blockaddress常量派生。

地址参数是要跳转到的label的地址。 其余参数表示地址可能指向的完整可能目的地的集合。 blocks允许多次出现在destination list中。
...
#+END_SRC
invoke指令使控制权转移到指定的function，控制流可能转移到normal label或exception label。如果callee 以ret指令返回，则控制流将返回normal label。如果callee（或任何间接callee）
通过resume指令或其他异常处理机制返回，则控制被中断并动态地在最近的exception label 处继续。

exception label是该exception的landing pad。因此，exception label必须有landingpad指令，该指令包含有关程序unwinding的行为的信息，作为它的第一条非PHI指令。

landing pad大致对应于在try/catch序列的catch部分中找到的代码。

这个指令需要如下一些参数：

1、可选的cconv参数表明使用哪种calling convention，没有指定则默认使用C calling convention。

2、可选的ret attrs表示返回值的属性，这里只有zeroext，signext，inreg属性可选。

3、可选的addrspace表示callee的address space。没有指定，则将使用datalayout string中的address space。

4、ty是call instruction自身的类型，同时也是返回值的类型，函数没有返回值则置为void。

5、fnty是被调函数的signature。参数类型必须和signature所暗含的类型相匹配。如果该函数不是可变长度参数，则此类型可以省略。

6、fnptrval:包含指针的llvm value，这个指针指向将要被调用的函数。

7、function args：类型与函数签名参数类型和参数属性匹配的参数列表。 所有参数必须是first class的类型。如果函数签名指示函数接受可变数量的参数，则可以指定额外的参数。

8、normal label:当被调用的函数执行ret指令后，会执行的label。

9、exception label:当callee以resume指令或其它异常处理机制返回后，会执行的label。

10、可选的函数属性列表。

11、可选的operand bundles列表。

在大多数情况下，此指令均旨在执行标准的call调用。主要区别在于它与label建立了关联，runtime library可以使用该label来unwind stack（主要用于异常处理）。

该指令在具有析构函数的语言中使用，以确保在longjmp或抛出异常的情况下执行正确的清除。 此外，这对于以支持它们的高级语言来实现catch子句也很重要。
...
#+END_SRC
callbr指令使控制权转移到指定的function，并且可能将控制流转移到fallthrough标签或indirect标签之一。

该指令仅应用于实现gcc样式内联汇编的goto功能。其他任何用法都会导致IR verifier的错误。
...
8、fallthrough label：内联汇编结束在底部结束后会执行的label。

9、indirect labels：callee将控制权转移到fallthrough label以外的位置时到达的label。这些blockaddress常数也应该在function args列表中。

10、可选的函数属性列表。

11、可选的operand bundles里表。

在大多数情况下，此指令均旨在执行标准的call 调用。主要区别在于它与其他label建立了关联，以定义控制流在调用之后的位置。

callbr指令的输出值仅可用于fallthrough block，而不适用于任何indirect block。

此功能的唯一用途是实现gcc内联汇编的goto功能，其中可以提供其他标签作为内联汇编跳转到的位置。
...
resume <type> <value>
#+END_SRC
resume指令是没有后续指令的终止指令。

resume指令需要一个参数，该参数必须与同一function中任何landingpad指令的结果具有相同的类型。

resume指令可恢复现有（运行中）异常的传递，该异常的展开已被landingpad指令中断。
...
#+END_SRC
LLVM的异常处理系统使用catchswitch指令来描述由EH personality 函数执行的可能的catch handlers。

parent参数是包含catchswitch指令的funclet的token。如果catchswitch不在funclet内，则此操作数可以是token none。

default参数是以cleanuppad或catchswitch指令开头的另一个basic block的label。此unwind destination必须是相对于parent links的合法目标。

处理程序是后继程序块的非空列表，每个后继程序块以catchpad指令开头。

执行此指令将控制权转移到处理程序中的后继者之一，或者继续通过unwind label（如果存在）进行unwind。

catchswitch既是terminator指令又是pad指令，这意味着它既是基本块中的第一条非phi指令，也必须是最后一条指令。 因此，它必须是该块中唯一的非phi指令。
...
catchret的第一个参数表示它退出哪个catchpad。它必须是一个catchpad。 catchret的第二个参数指定控制权将从何处转移到下一个。

catchret指令结束了一个现有的（运行中）异常，该异常的展开已被catchpad指令中断。personality function有机会执行任意代码，例如销毁active exception。然后控制权转移到正常状态。

token参数必须是catchpad指令产生的token。
...
#+END_SRC
cleanupret 具有可选的后继指令

cleanupret指令需要一个参数，该参数指示其退出哪个cleanuppad，并且必须是cleanuppad。

cleanupret指令还有一个可选的后继程序，continue，它必须是另一个以cleanuppad或catchswitch指令开头的基本块的label。

cleanupret指令向personality function指示其转移了控制权的一个cleanuppad已结束。 它转移控制权以继续或退出该function。
...
unreachable
#+END_SRC
“无法到达”指令没有定义好的语义。该指令用于通知optimizer，代码的特定部分不可访问。

** Unary Operations
一元运算符需要一个操作数，对其执行一个操作，并产生一个值。 操作数可能表示多个数据，向量数据类型就是这种情况。 结果值与其操作数具有相同的类型。
...
产生的值是操作数的副本，其符号位被翻转。该指令还可以采用任意数量的fast-math flags，这些flags是用于进行不安全的浮点优化的优化hints。
...
** Binary Operations
二元运算符用于执行程序中的大多数计算。它们需要两个相同类型的操作数，对它们执行一个操作，产生一个值。与向量数据类型一样，操作数可能表示多个数据。结果值与其操作数具有相同
的类型。
...
两个参数必须是整数或整数向量，两个参数必须有相同的类型。

如果总和产生无符号溢出，则返回的结果是对2^n求模的结果，其中n是结果的位宽。

由于LLVM整数使用二进制补码表示，因此该指令适用于有符号和无符号整数。

nuw和nsw分别代表“no unsigned wrap”和“no signed wrap”。 如果存在nuw和/或nsw关键字，那么如果分别发生无符号和/或有符号的溢出，则加法的结果值为poison value。
...
两个参数必须是浮点型或浮点型向量，两个参数必须有相同的类型。

假定该指令在默认floating-point environment中执行。 该指令还可以采用任意数量的快速运算符标志，这些标志是用于进行不安全的浮点优化。
...
如果差值产生无符号溢出，则返回的结果是对2^n求模的结果，其中n是结果的位宽。
...
如果乘积产生无符号溢出，则返回的结果是对2^n求模的结果，其中n是结果的位宽。
...
#+END_SRC
udiv指令返回两个操作数的无符号整数商。

两个参数必须是整数或整数向量，两个参数必须有相同的类型。

无符号整数除法和有符号整数除法是不同的操作。对于有符号整数除法，使用sdiv。

除以零是未定义的行为。 对于向量，如果除数的任何元素为零，则该操作是未定义的行为。

如果存在exact关键字，则如果％op1不是％op2的倍数，则udiv的结果值将是poison。
...
#+END_SRC
产生的值是两个操作数的有符号整数商，向0舍入。

除以零是未定义的行为。对于向量，如果除数的任何元素为零，则该操作具有未定义的行为。溢出还会导致未定义的行为。

如果出现exact关键字，结果存在舍入，则sdiv的结果值为poison value。
...
#+END_SRC
urem指令返回两个操作数无符号除法的余数。

两个参数必须是整型或整型向量，两个参数必须有相同的类型。

除以零求余是未定义的行为。对于向量，如果除数的任何元素为零，则该操作具有未定义的行为
...
#+END_SRC
srem指令返回其两个操作数的有符号除法的余数。

srem指令的两个参数必须是整数或整数值的向量。两个参数必须具有相同的类型。

结果为零或与被除数op1具有相同的符号，（模运算符结果为零或与除数op2具有相同的符号）。

除以零求余是未定义的行为。对于向量，如果除数的任何元素为零，则该操作具有未定义的行为。溢出还会导致未定义的行为。
...
这与libm'fmod'函数的输出相同，但是没有设置errno的可能性。 余数同被除数有相同的符号。假定该指令在默认floating-point environment中执行。
该指令还可以采用任意数量的快速运算符标志，这些标志是用于进行不安全的浮点优化。
...
按位二元运算符用于在程序中进行各种位翻转操作。 它们需要两个相同类型的操作数，对其执行一个操作，并产生一个值。 结果值与其操作数的类型相同。
...
shl指令的两个参数必须同时是整型或者整型向量，op2被视作无符号值。

产生的值是op1 * 2^op2 mod 2^n，其中n是结果的宽度。 如果op2（静态或动态）等于或大于op1中的位数，则该指令返回poison value。 如果参数是向量，则op1的每个向量元素都将偏移op2中的相应偏移量。

如果存在nuw关键字，若移位会移出任何非零位，则产生poison value。 如果存在nsw关键字，若该移位将移出与结果符号位不同的所有位，则产生poison value。
...
#+END_SRC
lshr指令（向右逻辑移位）将操作数向右移指定位数，用0填充。

两个参数必须同时是整型或者整型向量，op2被视作无符号值。

如果出现exact关键字，则如果移出的任何位都不为零，则lshr的结果值为poison value。
...
#+END_SRC
ashr指令（算术右移）将操作数向右移了指定位数，用符号位填充。
...
and指令对两个操作数执行按位逻辑与操作。

两个操作数必须同时为整型或整型向量，两个向量必须有相同的类型。
...
#+END_SRC
xor指令对两个操作数执行按位异或操作，它被用来实现补码操作。

两个操作数必须同时为整型或整型向量，两个向量必须有相同的类型。

** Vector Operations
LLVM支持几种指令，以独立于target的方式进行向量运算。这些指令涵盖了有效处理向量所需的成员访问和向量特定操作。
...
#+END_SRC
extractelement指令根据index从向量中提取单个标量元素。

第一个参数是一个向量值，第二个参数表示index，index可以是任何整型变量。

result是和val相同类型的标量，如果idx超过向量的长度，则result是一个poison值。对于可扩展向量，如果idx超过向量的运行时长度，则result是一个poison值。
...
#+END_SRC
insertelement在给定idx处插入一个标量到向量中。

第一个参数是一个向量值，第二个参数是要插入的标量值，它的类型必须和向量的成员类型相同，第三个操作数是index，表示标量要插入的位置。

结果是与val相同类型的向量。它的元素值是val的那些元素值，除了在idx位置，它的值是elt。 如果idx超过固定长度向量val长度，则结果是poison value。对于scalable向量，如果idx的值
超过向量的运行时长度，则结果为poison value。
...
#+END_SRC
shufflevector指令从两个输入向量构造元素的排列，返回一个向量，该向量具有与输入相同的元素类型，且长度与shuffle mask相同。

shufflevector指令前两个参数是两个相同类型的向量，第三个参数是成员类型为i32的shuffle mask向量。shuffle mask 向量的成员必须是常数整数或者undef值。
...
#+END_SRC
extractvalue指令从aggregate值中提取成员字段的值。

extractvalue指令的第一个操作数是struct或数组类型的值。其他操作数是常量索引，用于指定要提取的值，这个方式类似于getelementptr指令中的索引。

它和getelementptr索引的主要区别：由于要索引的值不是指针，因此将省略第一个索引并将其假定为零；必须至少指定一个索引；不仅struct索引而且数组索引都必须在范围内。
...
#+END_SRC
insertvalue指令将一个值插入到aggregate value的成员字段中。

insertvalue指令的第一个操作数是结构或数组类型的值。 第二个操作数是要插入的first class值。接下来的操作数是常量索引，用于指示要插入值的位置，其位置类似于extractvalue
指令中的索引。 要插入的值必须与索引所标识的值具有相同的类型。
...
#+END_SRC
alloca指令在当前执行的函数的堆栈帧上分配内存，当该函数返回其调用者时将自动释放该内存。始终在地址空间中为datalayout中指示的对象分配资源。

alloca指令在运行时堆栈上分配sizeof（<type>*NumElements个字节的内存，并向程序返回适当类型的指针。如果给定NumElements的值，则它为分配的元素数，否则NumElements默认为1。
如果指定了alignment，则分配的值结果至少与该边界对齐。alignment不得大于1 <<29。如果未指定，或者为零，则target可以选择与该类型兼容的对齐方式对齐。

分配的内存是未初始化的，并且从未初始化的内存加载会产生未定义的值。如果分配的堆栈空间不足，则操作本身是不确定的。函数返回时，会自动释放已分配的内存。
alloca指令通常用于表示必须具有可用地址的自动变量。当函数返回时（使用ret或resume指令），将回收内存。分配零字节是合法的，但是返回的指针可能不是唯一的。
没有指定分配内存的顺序（即堆栈增长的方式）。
...
#+END_SRC
load 指令的参数指定它应该load的地址，指定的类型必须是size已知的first class类型。如果将load标记为volatile，则不允许优化器使用其他volatile操作来修改此load的执行次数或执行顺序。

如果load标记为atomic load，则需要额外的排序和可选的syncscope（“ <target-scope>”）参数。release和acq_rel排序在load指令上无效。当automic load看到多个automic store时，
它们会产生确定的结果。pointee的类型必须是整数，指针或浮点类型，其位宽度是2的幂次，它大于或等于8且小于或等于target-specific size limit。必须在automic load上显式指定align，
并且如果未将对齐设置为至少大于pointee类型的字节大小的值，则load具有未定义的行为。!nontemporal对atomic load没有具体含义。

可选的constant align参数指定操作的对齐方式（即内存地址的对齐方式）。值为0或省略的align参数意味着该操作的对齐方式和target的ABI一样。高估alignment会导致不确定的行为。
低估alignment可能会产生低效的代码。alignment取1始终是安全的。可能的最大对齐方式是1 <<29。

可选的!nontemporal元数据必须引用一个元数据名称<nontemp_node>，该名称与包含一个i32类型的值为1的元数据节点相对应。指令中!nontemporal元数据的存在告诉优化器和代码生成器，此load
不会在缓存中重用。代码生成器可以选择特殊指令以节省缓存带宽，例如x86上的MOVNT指令。

可选的!invariant.load元数据必须引用与没有条目的元数据节点相对应的单个元数据名称<empty_node>。如果执行了以！invariant.load元数据标记的加载指令，则优化器可能会假定加载所引用的内存位置在程序中所有已知可取消引用的位置上都包含相同的值；否则，行为是不确定的。

可选的！invariant.group元数据必须引用一个元数据名称
<empty_node>对应于没有条目的元数据节点。请参见invariant.group元数据invariant.group。
可选的！nonnull元数据必须引用与没有条目的元数据节点相对应的单个元数据名称<empty_node>。指令中！nonnull元数据的存在告诉优化器，已知加载的值永远不会为null。如果在运行时该值为null，则行为是未定义的。这类似于参数和返回值的nonnull属性。此元数据只能应用于指针类型的负载。

可选的！dereferenceable元数据必须引用与具有一个i64条目的元数据节点相对应的单个元数据名称<deref_bytes_node>。请参阅可解除引用的可解除引用的元数据。

可选的！dereferenceable_or_null元数据必须引用与具有一个i64条目的元数据节点相对应的单个元数据名称<deref_bytes_node>。请参见dereferenceable_or_null元数据dereferenceable_or_null。

可选的！align元数据必须引用与具有一个i64条目的元数据节点相对应的单个元数据名称<align_node>。指令中！align元数据的存在告诉优化器，已知加载的值与元数据节点中整数值指定的边界对齐。对齐方式必须是2的幂。这类似于参数和返回值的'align'属性。此元数据只能应用于指针类型的负载。如果返回的值在运行时未正确对齐，则行为未定义。

可选的！noundef元数据必须引用与没有条目的节点相对应的单个元数据名称<empty_node>。指令中！noundef元数据的存在告诉优化器，已知加载的值定义正确。如果该值定义不正确，则行为是不确定的。
* lazy symbol binding
